...
def otp_login():
    ...
    if not otp_token:
        return jsonify({'error': 'OTP verification token is required.'}), 400

    # Call MSG91 to verify the access token
    import requests
    import secrets
    try:
        # MSG91 Auth key from user request
        msg91_authkey = "544626TwrWbOSvXspu6a3cee62P1"
        
        # Verify access token
        verify_url = "https://api.msg91.com/api/v5/widget/verifyAccessToken"
        verify_headers = {
            "authkey": msg91_authkey,
            "Content-Type": "application/json"
        }
        verify_body = {
            "access-token": otp_token
        }
        
        response = requests.post(verify_url, headers=verify_headers, json=verify_body, timeout=10)
        verify_data = response.json()
        
        if not response.ok or verify_data.get('type') != 'success':
            return jsonify({'error': verify_data.get('message') or 'Failed to verify OTP token with MSG91.'}), 401
            
        # Extract identifier
        user_info = verify_data.get('data') or {}
        if not isinstance(user_info, dict):
            user_info = {}
            
        identifier = (
            user_info.get('mobile') or 
            user_info.get('email') or 
            verify_data.get('mobile') or 
            verify_data.get('email') or 
            verify_data.get('identifier')
        )
        
        if not identifier:
            return jsonify({'error': 'No mobile number or email returned from verification.'}), 400
            
        # Clean identifier (remove leading '+' if any, strip)
        ...
        if not user:
            ...
            # Set a random password since they login with OTP
        ...
        return jsonify({'error': f'OTP Verification server error: {str(e)}'}), 500
